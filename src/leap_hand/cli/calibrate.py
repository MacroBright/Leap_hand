...
def main(argv=None):
    import argparse
    parser = argparse.ArgumentParser(description="LEAP Hand zero-point calibration and pose recording tool")
    parser.add_argument("--port", type=str, default=None, help="Serial port (e.g. /dev/ttyUSB0)")
    parser.add_argument("--action", choices=["1", "2"], default=None, help="1=Calibrate open pose, 2=Record custom pose")
    ...
    try:
        leap = LeapNode(port=args.port, calib_mode=True)
