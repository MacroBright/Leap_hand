...
try:
    from ..controller import LeapNode, OPEN_POSE, get_default_config_path
except (ImportError, ValueError):
    sys.path.insert(0, str(Path(__file__).resolve().parent.parent))
    from main import LeapNode, OPEN_POSE
    def get_default_config_path(fn="motor_limits.json"):
...
def main(argv=None):
    parser = argparse.ArgumentParser(description="LEAP Hand 16-motor physical limits diagnostics scanner")
    parser.add_argument("--port", type=str, default=None,
                        help="串口 (默认自动搜索 ttyUSB0/1, COM13)")
    parser.add_argument("--out", type=str, default=None,
                        help="输出 json 路径 (默认 configs/motor_limits.json)")
    ...
    out = args.out or str(get_default_config_path("motor_limits.json"))
