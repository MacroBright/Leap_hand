...
try:
    ...
    from ..vision.calibrator import LEAP_GROUP_LABELS, HUMAN_FINGER_LABELS
...
def draw_hud(image, angles, calibrator, bent_finger, bend_scores, show_diag=False,
             cur_joint=None, joint_gain=None, is_locked=False):
    ...
    cv2.putText(image, "SPACE=calib | L=lock/unlock | D=diag | TAB=joint | [ ]=gain | Q=quit",
...
def main(argv=None):
    parser = argparse.ArgumentParser(description="LEAP Hand real-time vision gesture teleoperation")
    ...
    gain_path = Path(__file__).resolve().parent.parent / "configs" / "joint_gain.json"
    ...
    print("  SPACE=calib | L=lock/unlock | D=diag | TAB=joint | [ ]=gain | S=save | Q=quit")
    print("=" * 50)
    print_motor_mapping()
    ...
    if args.drive:
        print("\n" + "=" * 62)
        print("  [🛡️ 真机安全延迟上电模式已启用]")
        print("  当前处于【未上电安全待命】状态以防误捕捉未准备画面。")
        print("  ▶ 请将右手完全伸直平摊面向摄像头；")
        print("  ▶ 按下键盘 [SPACE 空格键] 进行手势标定并正式上电跟随！")
        print("=" * 62 + "\n")
        ...
        while True:
            ...
            if results:
                ...
                if frame_count % 20 == 0:
                    ...
                    if is_locked:
                        print("  >>> 🔒 【姿态已锁定中】 灵巧手保持锁定状态 (按 L 解锁) <<<")

            else:
                if frame_count % 30 == 0:
                    print("  (no hand detected)" if not is_locked else "  (no hand detected - POSE LOCKED 🔒)")
                if leap is not None:
                    if is_locked and locked_pose is not None:
                        leap.set_leap(locked_pose)
                    else:
            ...
            if not args.no_display:
                if is_locked:
                    # 姿态锁定状态提示横幅
                    cv2.rectangle(frame, (10, h - 50), (450, h - 15), (0, 140, 255), -1)
                    cv2.putText(frame, "LOCKED: Pose Held (Press L to Unlock)",
                                (18, h - 25), cv2.FONT_HERSHEY_SIMPLEX,
                                0.58, (255, 255, 255), 2)
                elif args.drive and leap is None:
                    # 真机未上电安全待命提示
                    cv2.putText(frame, "STANDBY: Press SPACE to Calibrate & Power On",
                ...
                if key in (ord("q"), 27):
                    ...
                    if is_locked:
                        if results:
                            locked_pose = (OPEN_POSE + JOINT_DIR * angles).copy()
                        elif leap is not None:
                            locked_pose = leap.curr_pos.copy()
                        else:
                            locked_pose = OPEN_POSE.copy()
                        print("\n  🔒 [姿态已锁定] 灵巧手保持当前姿态！即使人手移开也保持不变。(再次按 L 解锁)\n")
                    else:
                        locked_pose = None
                        angle_filter.reset()
                        print("\n  🔓 [姿态已解锁] 恢复实时手势遥操跟随！\n")
                    ...
                    if results and results[0] is not None:
                        # 延迟上电保护: 首次按 SPACE (手张开对准相机) 时才初始化并上电驱动
                        if args.drive and leap is None:
                            try:
                                leap = LeapNode(port=args.port)
                                print("\n  [INFO] ⚡ LEAP Hand 硬件已成功初始化并上电使能跟随！\n")
                        ...
                        print("\n  [提示] 画面中未检测到手部，请将右手完全展平在摄像头前再按空格！\n")
