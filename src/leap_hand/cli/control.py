...
try:
    from ..controller import LeapNode, POSES, OPEN_POSE
...
def main(argv=None):
    import argparse
    parser = argparse.ArgumentParser(description="LEAP Hand interactive CLI controller")
    parser.add_argument("--port", type=str, default=None, help="Serial port (e.g. /dev/ttyUSB0)")
    args = parser.parse_args(argv)

    print("\n启动 LEAP Hand 控制器...")
    try:
        ctrl = HandController(port=args.port)
